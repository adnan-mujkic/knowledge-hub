...
namespace knowledge_hub.WebAPI.Services
{
    public class OrderService : CRUDService<OrderResponse, OrderInsertRequest, OrderUpdateRequest, Database.Order>, IOrderService
    {
        ...
        {
            var order = await _dbContext.OrderAddresses
        ...
        public async Task<List<OrderResponse>> InsertMany(OrderInsertRequest request)
        {
            ...
            for (int i = 0; i < request.Books.Count; i++)
            {
                ...
                {
                    _dbContext.Add(newOrder);
                    var newOrderAddress = new OrderAddress();
                    newOrderAddress.Comment = "";
                    newOrderAddress.CityId = request.CityId;
                    newOrderAddress.AddressLine = request.AddressLine;
                    newOrderAddress.OrderId = newOrder.OrderId;
                    _dbContext.Add(newOrderAddress);
                }
        ...
        {
            var order = await _dbContext.OrderAddresses.Where(x => x.OrderId == ID).FirstOrDefaultAsync();
            if (order == null) return null;

            order.Comment = request.Comment;
            order.Order.OrderStatus = request.OrderStatus;
            if (order.Order.OrderStatus == 1) order.Order.ShippingDate = DateTime.UtcNow;
